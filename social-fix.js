...
(function(){
  ...
  function wireCounters(){['meBox','profileBox'].forEach(id=>{const box=$(id);if(!box)return;const uid=profileId(box);box.querySelectorAll('*').forEach(el=>{if(el.dataset.dinoFollowWired)return;const type=findType(el);if(!type||id==='profileBox'&&!uid)return;el.dataset.dinoFollowWired='1';el.classList.add('dinoCountLink');el.addEventListener('click',async e=>{e.preventDefault();e.stopPropagation();const u=id==='meBox'?await current():null;await showFollowListFixed(u?.id||uid,type)},true)})})}
