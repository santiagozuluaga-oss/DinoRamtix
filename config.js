...
(function(){
  ...
  // Profile photo upload: compresses the selected image and stores it in avatar_url.
  // This avoids requiring a separate Storage bucket and makes the photo visible everywhere.
  function installProfilePhotoEditor(){
    ...
    window.editProfile=async function(){
      ...
      $('meBox').innerHTML=`<div class="card"><h2>Editar perfil</h2>
        <div style="display:flex;align-items:center;gap:16px;margin-bottom:12px">${window.av(u)}
          <div><b>Foto de perfil</b><div class="muted">Elige una imagen desde tu dispositivo.</div></div></div>
        <input id="avatarFile" type="file" accept="image/png,image/jpeg,image/webp" onchange="previewAvatar(this)">
        <div id="avatarPreview" style="margin:10px 0"></div>
        <label>Nombre de usuario</label><input id="eu" value="${window.esc(u.username)}">
        <label>Biografía</label><textarea id="eb">${window.esc(u.bio||'')}</textarea>
        <button class="p" onclick="saveProfile()">Guardar cambios</button>
        <button class="g" onclick="my()">Cancelar</button></div>`;
    };
    window.previewAvatar=function(input){
      const file=input.files?.[0]; if(!file)return;
      if(file.size>8*1024*1024)return alert('La imagen debe pesar menos de 8 MB.');
    ...
    window.saveProfile=async function(){
      const username=$('eu').value.trim(),bio=$('eb').value.trim();
      if(!username)return alert('Escribe un nombre de usuario.');
      const patch={username,bio}; if(window._pendingAvatar)patch.avatar_url=window._pendingAvatar;
      const r=await window.db.from('profiles').update(patch).eq('id',window.me.id);
      if(r.error)return alert(r.error.message);
      window._pendingAvatar=null; await originalMy();
    };
    return true;
  }
